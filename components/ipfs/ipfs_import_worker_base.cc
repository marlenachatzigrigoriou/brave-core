...
const char kBraveImportDirectory[] = "/brave-imports/";
...
void IpfsImportWorkerBase::OnImportAddComplete(
  ...
  if (success) {
    ...
    success = parts.size() && IPFSJSONParser::GetImportResponseFromJSON(
                                  parts.front(), data_.get());
  }
  url_loader_.reset();
...
void IpfsImportWorkerBase::CreateBraveDirectory() {
  ...
  std::string directory = kBraveImportDirectory;
