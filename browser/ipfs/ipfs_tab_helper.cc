...
void IPFSTabHelper::ImportTextToIpfs(const std::string& text) {
  ...
  service->ImportTextToIpfs(text, web_contents()->GetURL().host(),
                            base::BindOnce(&IPFSTabHelper::OnImportCompleted,
                                           weak_ptr_factory_.GetWeakPtr()));
}
...
void IPFSTabHelper::OnImportCompleted(const ipfs::ImportedData& data) {
  ...
  GURL url = GetAPIServer(chrome::GetChannel());
  GURL::Replacements replacements;
  replacements.SetPathStr("/webui/");
  std::string webui_files_ref = std::string("/files") + data.directory;
  replacements.SetRefStr(webui_files_ref);

  content::OpenURLParams params(url.ReplaceComponents(replacements),
                                content::Referrer(),
